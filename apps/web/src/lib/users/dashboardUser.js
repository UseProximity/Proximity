...
// Every `users` column that is safe to return to the client. Deliberately
// EXCLUDES the credential/verification columns that exist on the table:
...
// This function's result is serialized straight to the browser and the mobile
// app by both /api/getUser and /api/admin/viewUser, so a wildcard select here
// ships a working password-reset token in a JSON body — an account-takeover
// primitive. Allowlisting also fails closed: a future sensitive column is
// excluded by default rather than leaked. Keep in sync when adding a column.
 ...
 *
 * There are two doors onto the same screen: /api/getUser, which serves the
 * person who logged in, and /api/admin/viewUser, which serves an admin looking
 * at somebody else through "view as". They rendered the same components, so
 * they had to return the same shape, and for a while they did: viewUser began
 * as a copy of getUser.
 *
 * The copy then fell five months behind, and because both halves still ran
 * without erroring nothing ever announced it. By the time it was found, an
 * admin viewing as a landlord was reading a different building than the
 * landlord was. At Kingsland Courtyard the copy showed six 4-bed units, every
 * one of them flagged hidden and carrying no offering; the landlord's own
 * dashboard and browse both showed the one live unit and its lease. Five of
 * those six rows were soft-deleted duplicates the copy's select never filtered,
 * and it never joined unit_leases at all, so there were no offerings to show
 * and no `available` to read.
 *
 * That is the whole reason this module exists. View-as is not a second
 * rendering of the dashboard, it is the same one pointed at another user, and
 * the only way to keep it honest is to leave it no query of its own to drift.
 * Everything the dashboard reads is built here. The routes are doors: they
 * decide WHO may be looked at, and nothing about what is returned.
 */
...
/**
 * Build the dashboard payload for one user, identified however the caller
 * happens to hold them.
 *
 * `email` is how a logged-in session identifies itself, since provider ids
 * differ between auth methods and the email does not. `id` is how an admin
 * names somebody else. They resolve to the same row and the same payload; only
 * the lookup differs.
 *
 * Returns null when there is no such user, so the caller can 404 it.
 */
export async function buildDashboardUser({ email, id } = {}) {
  ...
  const query = supabase
    ...
    .select(`${USER_PUBLIC_COLUMNS}, roles!role_id(name)`);

  const { data: user, error: userError } = await (
    id ? query.eq("id", id) : query.eq("email", email)
  ).maybeSingle();
