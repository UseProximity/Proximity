...
/*
 * One Supabase client for the whole tab.
 *
 * createClient() opens its own Realtime socket, so calling it per subscription
 * would leave a tab holding one socket per open thread plus the inbox. The
 * module keeps a single instance and hands it to every caller, which also means
 * realtime.setAuth() from ensureRealtimeAuth() applies to every channel at once.
 *
 * Auth is deliberately inert here: sessions live in NextAuth, and the only token
 * this client ever carries is the short-lived Realtime JWT minted by
 * /api/chat/realtime-token. Persisting or auto-refreshing a Supabase session
 * would be a second, competing source of identity.
 */
