...
4c. RESIDENTIAL ONLY: Proximity lists residential student rentals. Exclude commercial, office, retail, industrial, land, and self-storage properties entirely, from "properties" (including group entries: never list a commercial/office section as a group) and from any listing, even when the site mixes them with rentals. When you can't tell, include it.
4b. GROUPS vs PROPERTIES: each entry gets a "kind". A neighborhood, area, city, or category page (e.g. "Central West End", "Clayton", "Our Communities") is kind "group", never a property; the landlord will open it to see the actual buildings inside. So is a site section that leads to the residential rentals when the buildings themselves aren't on the current page (e.g. "Apartments for Rent", "Available Rentals", "Our Properties" from CANDIDATE LINKS). An individual building or complex with its own name or street address is kind "property". When the page shows both (areas AND some buildings), list both with correct kinds. Never return an empty properties list when the site clearly has a rentals section you can point to as a group. Never invent a listing from a group page's teaser text.
5. UNITS are floor-plan types (e.g. "2 bed / 1 bath"), not physical apartments. Collapse repeats. "title" is the floor plan's marketing name if the site uses one (e.g. "The Loft"). STUDIOS: record a studio as bedrooms 0, and put "Studio" in the unit's title (unless the site gives it a specific name). AVAILABILITY: when the site states when a unit is available, set availableFrom: "now" for "available now/immediately", or the date as YYYY-MM-DD (infer the year sensibly for month-day dates: the next occurrence). Null when not stated.
