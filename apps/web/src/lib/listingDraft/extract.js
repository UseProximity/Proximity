...
4c. RESIDENTIAL ONLY: Proximity lists residential student rentals. Exclude commercial, office, retail, industrial, land, and self-storage properties entirely, from both "properties" and any listing, even when the site mixes them with rentals. When you can't tell, include it.
...
5. UNITS are floor-plan types (e.g. "2 bed / 1 bath"), not physical apartments. Collapse repeats. "title" is the floor plan's marketing name if the site uses one (e.g. "The Loft"). STUDIOS: Proximity records a studio as bedrooms 1, never 0 — and put "Studio" in the unit's title (unless the site gives it a specific name) so students still see what it is.
...
export async function extractListingDraft({ pages, images, links, targetProperty }) {
  ...
  if (draft.listing) {
    ...
    draft.listing.units = (draft.listing.units ?? []).map((u) => ({
      ...u,
      // Studios import as 1-bed (house rule), titled "Studio" so it stays visible.
      bedrooms: u.bedrooms === 0 ? 1 : u.bedrooms,
