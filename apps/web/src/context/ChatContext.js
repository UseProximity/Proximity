/*
 * In-memory chat state for the floating ChatWidget. Manages a list of open conversations,
 * which conversation is currently active, and whether the widget drawer is open. Each
 * conversation is typed as either "landlord" or "roommate" and starts with a canned
 * opening message. Replies are simulated with a random delay (1.5–3.5s) and pulled from
 * two static response pools — this is UI scaffolding until real messaging is wired to
 * Supabase's chat tables (migration 202604170021). openConversation() deduplicates by
 * profile name + type so clicking the same contact twice focuses the existing thread.
 * Consumed via useChatContext() by ChatWidget and any component that triggers a chat.
 */
"use client";
