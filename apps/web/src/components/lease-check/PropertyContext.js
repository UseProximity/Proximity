...
/*
 * Reviews + comps for a high-confidence match, plus the "Not the right place?"
 * correction affordance. When the match is low/none, ONLY the correction input
 * renders — showing another building's reviews is the one unforgivable bug here.
 */
export default function PropertyContext({ leaseCheckId, property, rentBasis, onProperty }) {
  ...
  const submitCorrection = async (e) => {
    ...
    try {
      ...
      if (data.property?.matchConfidence !== "high") {
        toast.error("Couldn't find that one on Proximity yet.");
      }
  ...
  const correctionForm = (
    <form onSubmit={submitCorrection} className="mt-3 flex gap-2">
      <input
        type="text"
        value={address}
      ...
      <button
        ...
        className="bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white font-semibold py-2.5 rounded-lg text-sm transition-colors px-5 shrink-0"
      >
    ...
    return (
      <div className="rounded-[1.75rem] border border-dashed border-gray-300 bg-gray-50 p-6">
        <p className="text-sm font-semibold text-gray-700">
          Living somewhere on Proximity? Tell us where.
        </p>
        <p className="mt-1 text-xs text-gray-500">
          We&apos;ll pull that building&apos;s reviews and check for cheaper places nearby.
        </p>
        {correctionForm}
      </div>
    );
