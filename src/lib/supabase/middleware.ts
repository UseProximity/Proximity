/*
 * Supabase client factory used exclusively by src/middleware.ts. Creates a server client
 * that can read and refresh the Supabase session cookie on every request before the page
 * renders. Also injects x-pathname and x-search as custom request headers so downstream
