...
export default function AdminDashboard() {
  ...
  // Per-ambassador expandable detail (their referred reviews)
  const [expandedReferrer, setExpandedReferrer] = useState(null);
  const [referrerDetails, setReferrerDetails] = useState({}); // id -> reviews[]
  const [detailsLoadingId, setDetailsLoadingId] = useState(null);

  async function toggleReferrerDetails(id) {
    if (expandedReferrer === id) {
      setExpandedReferrer(null);
      return;
    }
    setExpandedReferrer(id);
    if (!referrerDetails[id]) {
      ...
      try {
        ...
        setReferrerDetails((prev) => ({ ...prev, [id]: [] }));
      } finally {
                  ...
                  {referrals.map((r, i) => (
                    <div key={r.id} className="border border-gray-100 rounded-lg">
                      <div className="flex items-center justify-between gap-2 px-3 py-2">
                        <button
                          onClick={() => toggleReferrerDetails(r.id)}
                          className="flex items-center gap-2 min-w-0 flex-1 text-left"
                          title="Show referred reviews"
                        >
                          <svg className={`w-3.5 h-3.5 text-gray-400 transition-transform ${expandedReferrer === r.id ? "rotate-90" : ""}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                      ...
                      {expandedReferrer === r.id && (
                        <div className="border-t border-gray-100 px-3 py-2 bg-gray-50">
                          {detailsLoadingId === r.id ? (
                            <p className="text-xs text-gray-400 py-1">Loading…</p>
                          ) : (referrerDetails[r.id] || []).length === 0 ? (
                            <p className="text-xs text-gray-400 py-1">No reviews yet.</p>
                          ) : (
                            <div className="space-y-1">
                              {(referrerDetails[r.id] || []).map((rv) => (
                                <div key={rv.id} className="flex items-center justify-between gap-2 text-xs">
                                  <span className="min-w-0 truncate">
                                    <span className="font-medium text-gray-700">{rv.reviewerName}</span>
                                    {rv.reviewerEmail && <span className="text-gray-400"> ({rv.reviewerEmail})</span>}
                                  </span>
                                  <span className="text-gray-500 truncate text-right">{rv.address}</span>
                                </div>
                              ))}
                            </div>
                          )}
