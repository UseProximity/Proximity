-- Restore chat email re-notify cooldown to 24 hours.
-- 202608150005 accidentally set v_renotify_after to 30 minutes while adding messageType.
-- Cooldown remains per thread participant (last_notified_at on chat_participants), so
-- multiple students messaging the same listing still notify independently.
...
DO $$ BEGIN
  RAISE NOTICE 'Migration 202608150007: chat email re-notify cooldown restored to 24 hours.';
END $$;
