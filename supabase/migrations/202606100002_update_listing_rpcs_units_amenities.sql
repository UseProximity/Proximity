-- Extend the create/edit listing RPCs for: per-unit title + floor plan image,
-- custom per-lease term (months), free-text custom amenities, and (on create)
-- persisting the listings.lease_availability term array.

DROP FUNCTION IF EXISTS public.rpc_create_listing(uuid, jsonb, jsonb, jsonb, jsonb, jsonb, text);
...
BEGIN
  ...
  FOR v_unit IN SELECT * FROM jsonb_array_elements(p_units) LOOP
    ...
    v_term := NULLIF(v_unit->>'leaseTermMonths', '')::integer;

    IF v_rent IS NOT NULL OR v_avail IS NOT NULL OR v_sublease OR v_term IS NOT NULL THEN
      INSERT INTO unit_leases (unit_id, rent, is_active, available_from, sublease, lease_term_months)
      VALUES (v_unit_id, v_rent, true, v_avail, v_sublease, v_term);
    END IF;
  END LOOP;
...
DROP FUNCTION IF EXISTS public.rpc_edit_listing(uuid, uuid, jsonb, jsonb, jsonb, text[], jsonb, text, jsonb, text);
...
BEGIN
  ...
  -- Replace free-text custom amenities when provided
