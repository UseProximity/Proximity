-- Expose the other participant's last_read_at on the inbox RPC for read receipts.
-- Also publish chat_participants so the open thread can live-update "Read" via Realtime.
...
-- Live "Read" receipts: deliver participant last_read_at UPDATEs to the other party.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
